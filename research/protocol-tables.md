# Research — Raw Protocol Tables

Machine-extracted tables from smali analysis (register-backtracking). Kept for reproducibility.

---

## Table 1 — `acceptMsgTypes()` registrations (42 entries)

Source: `com.heytap.device.data.sporthealth.receive.*` (classes12.dex), extracted via register backtracking of `MsgProcessor$MsgType;->create(II)`.

| Processor | serviceId | commandId |
|-----------|-----------|-----------|
...
## Table 2 — `MessageEvent` construction sites (send side, 63 entries)

Source: classes12.dex, extracted via register backtracking of `MessageEvent;-><init>(II[B)`.

| Class (path fragment) | line | svc | cmd |
|-----------------------|------|-----|-----|
...
*(table truncated to unique class/line entries; duplicates from multi-path files merged)*

---

## Notes

- Extracted with a Python register-backtracking script over baksmali output (classes2–14 available at analysis time).
- `svc`/`cmd` values here are **as-mounted on the constructor**: first int argument → `mServiceId`, second → `mCommandId` (per `MessageEvent.<init>(II[B)` field stores, verified on multiple samples).
- Only entries with ≥2 resolvable constants within the method window are listed; malformed/ambiguous sites were excluded.