...
1. **The Watch speaks a proprietary message protocol over OHealth's `linkservice` stack**. Current research found **no evidence** of these messages being carried over a standard GATT profile (the link was observed at the OS level as classic BT BR/EDR). Messages are `MessageEvent(serviceId, commandId, byte[])` carrying protobuf payloads, with a callback-based request/response mechanism (`waitResponseMsgMap`, key = serviceId+commandId concat).
